...
>[!WARNING] These only make sense at a stationary point, thus we have to use the minimum energy structure located using the same >methodology. Let's prepare `uracil_s0_freq.inp` based on a copy of input for geometry optimization:
